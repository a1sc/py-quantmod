...
def add_MA(self, timeperiod=20, matype=0, type="line", color="secondary", **kwargs):
...
def add_SMA(self, timeperiod=20, type="line", color="secondary", **kwargs):
...
def add_MA_3(self, timeperiod=3):
    self.add_SMA(timeperiod=timeperiod)


def add_MA_6(self, timeperiod=6):
    self.add_SMA(timeperiod=timeperiod, color="tertiary")


def add_MA_50(self, timeperiod=50):
    self.add_SMA(timeperiod=timeperiod, color="grey_strong")


def add_EMA(self, timeperiod=26, type="line", color="secondary", **kwargs):
...
def add_DEMA(self, timeperiod=26, type="line", color="secondary", **kwargs):
...
def add_TEMA(self, timeperiod=26, type="line", color="secondary", **kwargs):
...
def add_MAMA(
    ...
    **kwargs
    ...
    self.ind[mama], self.ind[fama] = talib.MAMA(
        self.df[self.cl].values, fastlimit, slowlimit
    )
...
def add_BBANDS(
    ...
    **kwargs
    ...
    name = "BBANDS({},{},{})".format(str(timeperiod), str(nbdevup), str(nbdevdn))
    ubb = name + "[Upper]"
    bb = name
    lbb = name + "[Lower]"
    self.pri[ubb] = dict(type="line_" + types[0][5:], color=colors[0])
    self.pri[bb] = dict(type="area_" + types[1][5:], color=colors[1], fillcolor="fill")
    self.pri[lbb] = dict(type="area_" + types[0][5:], color=colors[0], fillcolor="fill")
    (self.ind[ubb], self.ind[bb], self.ind[lbb]) = talib.BBANDS(
        self.df[self.cl].values, timeperiod, nbdevup, nbdevdn, matype
    )


def add_HT_TRENDLINE(self, type="line", color="secondary", **kwargs):
...
def add_SAR(
    self, acceleration=0.02, maximum=0.20, type="scatter", color="tertiary", **kwargs
):
    """Parabolic SAR."""
    ...
    self.pri[name] = dict(type=type, color=color)
    self.ind[name] = talib.SAR(
        self.df[self.hi].values, self.df[self.lo].values, acceleration, maximum
    )
...
def add_APO(
    ...
    **kwargs
    ...
    self.ind[name] = talib.APO(self.df[self.cl].values, fastperiod, slowperiod, matype)
...
def add_AROONOSC(self, timeperiod=14, type="area", color="secondary", **kwargs):
    """Aroon Oscillator."""
...
def add_CCI(self, timeperiod=14, type="line", color="secondary", **kwargs):
...
def add_ADX(self, timeperiod=14, type="line", color="secondary", **kwargs):
...
def add_MINUS_DI(self, timeperiod=14, type="line", color="decreasing", **kwargs):
...
def add_PLUS_DI(self, timeperiod=14, type="line", color="increasing", **kwargs):
...
def add_MINUS_DM(self, timeperiod=14, type="line", color="decreasing", **kwargs):
...
def add_MACD(
    ...
    **kwargs
    ...
    name = "MACD({},{},{})".format(str(fastperiod), str(slowperiod), str(signalperiod))
    macd = name
    smacd = name + "[Sign]"
    hmacd = name + "[Hist]"
    ...
    (self.ind[macd], self.ind[smacd], self.ind[hmacd]) = talib.MACD(
        self.df[self.cl].values, fastperiod, slowperiod, signalperiod
    )
...
def add_MFI(self, timeperiod=14, type="line", color="secondary", **kwargs):
    """Money Flow Index."""

    if not (self.has_high and self.has_low and self.has_close and self.has_volume):
        raise Exception()
...
def add_MOM(self, timeperiod=10, type="line", color="secondary", **kwargs):
    """Momentum Indicator."""
    ...
    self.sec[name] = dict(type=type, color=color)
    self.ind[name] = talib.MOM(self.df[self.cl].values, timeperiod)
...
def add_PPO(
    ...
    **kwargs
    ...
    self.ind[name] = talib.PPO(self.df[self.cl].values, fastperiod, slowperiod, matype)


def add_ROC(self, timeperiod=10, type="line", color="tertiary", **kwargs):
    """Rate of Change."""
    ...
    self.sec[name] = dict(type=type, color=color)
    self.ind[name] = talib.ROC(self.df[self.cl].values, timeperiod)


def add_ROCP(self, timeperiod=10, type="line", color="tertiary", **kwargs):
...
def add_ROCR(self, timeperiod=10, type="line", color="tertiary", **kwargs):
    ...
    self.sec[name] = dict(type=type, color=color)
    self.ind[name] = talib.ROCR(self.df[self.cl].values, timeperiod)
...
def add_RSI(self, timeperiod=14, type="line", color="secondary", **kwargs):
...
def add_STOCHRSI(
    ...
    **kwargs
    ...
    name = "STOCHRSI({},{},{})".format(
        str(timeperiod), str(fastk_period), str(fastd_period)
    )
    fastk = name + r"[%k]"
    fastd = name + r"[%d]"
    self.sec[fastk] = dict(type=types[0], color=colors[0])
    self.sec[fastd] = dict(type=types[1], color=colors[1], on=fastk)
    self.ind[fastk], self.ind[fastd] = talib.STOCHRSI(
        self.df[self.cl].values, timeperiod, fastk_period, fastd_period, fastd_matype
    )
...
def add_WILLR(self, timeperiod=14, type="line", color="secondary", **kwargs):
    """Williams %R."""
